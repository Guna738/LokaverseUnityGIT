...
public class NL_AttachCargo : MonoBehaviour
{
    ...
    void Update()
    {
        if (inZone && rbTarget != null && !isAttached)
        {
        ...
        if (Application.isPlaying)
        {
            if (Input.GetKeyDown(attachKey) || UxrAvatar.LocalAvatarInput.GetButtonsEvent(UxrHandSide.Right, UxrInputButtons.Button1, UxrButtonEventType.PressDown))
            {
                if (!isAttached)
                {
