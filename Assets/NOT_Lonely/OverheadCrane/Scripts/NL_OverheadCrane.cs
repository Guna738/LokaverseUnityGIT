namespace NOT_Lonely
{
    ...
    public class NL_OverheadCrane : MonoBehaviour
    {
        ...
        public void MoveCraneForward(float angleSpeed)
        {
            if (initialBeamPos < 1 && angleSpeed >= 25)
            {
                initialBeamPos += beamSpeed * angleSpeed * 0.1f * Time.deltaTime / (craneLength - 2.316f);
                UpdateMainBeamPos(initialBeamPos);
            }
            else
            if (initialBeamPos > 0 && angleSpeed <= 25)
            {
                initialBeamPos -= beamSpeed * (-angleSpeed * 0.1f) * Time.deltaTime / (craneLength - 2.316f);
                UpdateMainBeamPos(initialBeamPos);
            }
        ...
        public void MoveCraneLeft(float angleSpeed)
        {
            if (initialTelpherPos < 1 && angleSpeed >= 25)
            {
                initialTelpherPos += telpherSpeed * angleSpeed * 0.1f * Time.deltaTime / (craneWidth - 1.57f);
                UpdateTelpherPos(initialTelpherPos);
            }

            if (initialTelpherPos > 0 && angleSpeed <= 25)
            {
                initialTelpherPos -= telpherSpeed * (-angleSpeed * 0.1f) * Time.deltaTime / (craneWidth - 1.57f);
                UpdateTelpherPos(initialTelpherPos);
            }
